...
public class NearbySchedulesResource {
    ...
    public Collection<StopTimesByStop> getNearbySchedules() {
        ...
        Map<FeedScopedId, StopTimesByStop> stopIdAndStopTimesMap = getStopTimesByParentStop(transitStops, startTime, transitStopStates);
    ...
    private Map<FeedScopedId, StopTimesByStop> getStopTimesByParentStop(Collection<TransitStop> transitStops, long startTime, Map<TransitStop, State> stateMap){
        Map<FeedScopedId, StopTimesByStop> stopIdAndStopTimesMap = new LinkedHashMap<>();
        RouteMatcher routeMatcher = RouteMatcher.parse(routesStr);
        for (TransitStop tstop : transitStops) {
            Stop stop = tstop.getStop();
            FeedScopedId key = key(stop);
    ...
    private FeedScopedId key(Stop stop) {
    ...
    private List<TransitStop> getStopsFromList(String stopsStr) {
        ...
        for (String st : stopsStr.split(",")) {
            FeedScopedId id = FeedScopedId.convertFromString(st, ':');
