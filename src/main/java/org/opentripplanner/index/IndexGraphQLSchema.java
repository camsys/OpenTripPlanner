...
public class IndexGraphQLSchema {
    ...
    public GraphQLOutputType pageInfo = new GraphQLTypeReference("PageInfo");
    ...
    private GraphQLInterfaceType nodeInterface = relay.nodeInterface(new TypeResolver() {
		...
		public GraphQLObjectType getType(TypeResolutionEnvironment env) {
            if (env.getObject() instanceof StopCluster){
                return (GraphQLObjectType) clusterType;
            }
            if (env.getObject() instanceof Stop){
                return (GraphQLObjectType) stopType;
            }
            if (env.getObject() instanceof Trip){
                return (GraphQLObjectType) tripType;
            }
            if (env.getObject() instanceof Route){
                return (GraphQLObjectType) routeType;
            }
            if (env.getObject() instanceof TripPattern){
                return (GraphQLObjectType) patternType;
            }
            if (env.getObject() instanceof Agency){
	...
	public IndexGraphQLSchema(GraphIndex index) {
        ...
        feedType = GraphQLObjectType.newObject()
                ...
                .field(GraphQLFieldDefinition.newFieldDefinition()
                        ...
                        .type(new GraphQLNonNull(Scalars.GraphQLString))
                        .dataFetcher(environment -> index.graph.getFeedInfo(environment.getSource()).getVersion())
                        .build())
                .field(GraphQLFieldDefinition.newFieldDefinition()
                        .name("agencies")
        ...
        queryType = GraphQLObjectType.newObject()
            ...
            .field(GraphQLFieldDefinition.newFieldDefinition()
                    ...
                    .description("Version of the feed")
                    .type(new GraphQLNonNull(Scalars.GraphQLString))
                    .dataFetcher(environment -> index.graph.getFeedInfo(environment.getArgument("feedId")).getVersion())
                    .build())
            .field(GraphQLFieldDefinition.newFieldDefinition()
                .name("agencies")
