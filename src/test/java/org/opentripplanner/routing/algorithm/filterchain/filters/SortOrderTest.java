...
/**
 * Base class for SortOrder testing.
 */
public abstract class SortOrderTest implements PlanTestConstants {
  protected List<Itinerary> result;

  protected void runSortStreetBeforeTransitThenTime() {
  ...
  protected void runSortOnGeneralizedCostVsTime() {
  ...
  protected void runSortOnGeneralizedCostVsNumberOfTransfers() {
  ...
  protected void runSortOnTransfersVsTime() {
    Itinerary iA = newItinerary(A).bus(21, 0, 20, G).build();
    iA.generalizedCost = 1;
  ...
  protected String toStr(Itinerary... list) {
    return Itinerary.toStr(Arrays.asList(list));
  }

  protected String toStr(List<Itinerary> list) {
    return Itinerary.toStr(list);
  }

  protected abstract SortFilter arriveBySort();

  protected abstract SortFilter departAfterSort();
